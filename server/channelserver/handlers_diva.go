...
type KijuInfo struct {
	Name        string `db:"name"`
	Description string `db:"description"`
	Color       uint8  `db:"color"`
	Effect      uint8  `db:"effect"`
}
...
func handleMsgMhfGetKijuInfo(s *Session, p mhfpacket.MHFPacket) {
	...
	var effect1, effect2, effect3, effect4 uint8

	s.server.db.Exec("INSERT INTO demo_select_kiju(char_id, effect1, effect2, effect3, effect4) SELECT $1, 1, 8, 15, 18 WHERE NOT EXISTS(SELECT * FROM demo_color WHERE char_id = $1);", s.charID)

	rows, err := s.server.db.Queryx("SELECT effect1, effect2, effect3, effect4 FROM demo_select_kiju WHERE char_id = $1", s.charID)
	if err != nil {
		s.logger.Error("Error parse effect data", zap.Error(err))
	} else {
		for rows.Next() {
			err := rows.Scan(&effect1, &effect2, &effect3, &effect4)
			if err != nil {
				s.logger.Error("Error scanning effect data", zap.Error(err))
			}
		}
	}
	...
	rows, err = s.server.db.Queryx(`SELECT name, description, row_number() over() as color, effect FROM kiju_list WHERE effect in ($1, $2, $3, $4) ORDER BY color ASC;`, effect1, effect2, effect3, effect4)
	if err != nil {
		s.logger.Error("Error getting KijuInfo", zap.Error(err))
		doAckBufSucceed(s, pkt.AckHandle, make([]byte, 4))
	} else {
		for rows.Next() {
			count++
			kijuinfo := &KijuInfo{}
			err = rows.StructScan(&kijuinfo)
			if err != nil {
				s.logger.Error("Error scanning KijuInfo", zap.Error(err))
			}
			bf.WriteBytes(stringsupport.PaddedString(kijuinfo.Name, 32, true))
			bf.WriteBytes(stringsupport.PaddedString(kijuinfo.Description, 512, true))
			bf.WriteUint8(kijuinfo.Color)
			bf.WriteUint8(kijuinfo.Effect)
			if count == 4 {
				break
			}
		}
...
type ColorInfo struct {
	Color_1 uint8 `db:"color_1"`
	Color_2 uint8 `db:"color_2"`
	Color_3 uint8 `db:"color_3"`
	Color_4 uint8 `db:"color_4"`
}
...
func handleMsgMhfGetUdSelectedColorInfo(s *Session, p mhfpacket.MHFPacket) {
	...
	s.server.db.Exec("INSERT INTO demo_color(char_id, color_1, color_2, color_3, color_4) SELECT $1, 1, 3, 1, 2 WHERE NOT EXISTS(SELECT * FROM demo_color WHERE char_id = $1);", s.charID)

	rows, err := s.server.db.Queryx(`SELECT color_1, color_2, color_3, color_4 FROM demo_color WHERE char_id = $1`, s.charID)
	if err != nil {
		s.logger.Error("Failed to get ColorInfo from db", zap.Error(err))
	} else {
		for rows.Next() {
			colorinfo := &ColorInfo{}
			err = rows.StructScan(&colorinfo)
			if err != nil {
				s.logger.Error("Error scanning ColorInfo", zap.Error(err))
				doAckBufSucceed(s, pkt.AckHandle, []byte{0x00, 0x01, 0x02, 0x02, 0x02, 0x03, 0x04, 0x04, 0x00})
			}

			c1 := colorinfo.Color_1
			for c1 > 0 {
				bf.WriteUint8(1)
				c1--
			}
			c2 := colorinfo.Color_2
			for c2 > 0 {
				bf.WriteUint8(2)
				c2--
			}
			c3 := colorinfo.Color_3
			for c3 > 0 {
				bf.WriteUint8(3)
				c3--
			}
			c4 := colorinfo.Color_4
			for c4 > 0 {
				bf.WriteUint8(4)
				c4--
			}
		...
		resp := byteframe.NewByteFrame()
		resp.WriteUint8(0)
		resp.WriteBytes(stringsupport.PaddedString(string(bf.Data()), 8, false))
